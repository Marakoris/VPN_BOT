...
async def start_bot():
    ...
    # Resource check серверов — диск и RAM (каждые 30 минут)
    async def server_resource_check_job():
        await check_servers_resources(bot)

    scheduler.add_job(
        server_resource_check_job,
        trigger=IntervalTrigger(minutes=30),
        id="server_resource_check",
        replace_existing=True
    )
