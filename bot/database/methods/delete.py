...
from bot.database.models.main import Servers, StaticPersons, PromoCode, Groups, SuperOffer
...
async def drop_all(engine: AsyncEngine, Base: DeclarativeBase):
    async with engine.begin() as connection:
        await connection.run_sync(Base.metadata.drop_all)

async def delete_supper_offer():
    async with AsyncSession(autoflush=False, bind=engine()) as db:
        await db.execute(delete(SuperOffer))
        await db.commit()