...
# ========== Relaxed JSON Parser for x-ui SJSON format ==========

def relaxed_to_json(s: str) -> str:
    """Convert x-ui relaxed JSON (SJSON) to standard JSON.

    x-ui stores settings in SJSON format without quotes around keys:
...
def safe_json_loads(s: str) -> dict:
    """Parse JSON with fallback to relaxed JSON (SJSON) for x-ui compatibility"""
    try:
        return json.loads(s)
