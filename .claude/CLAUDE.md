...
## 📋 ПЛАН РАБОТ — Email-сервис для кабинета

Требуется настройка SMTP (Yandex/Gmail/SendGrid/Brevo) для следующих задач:

1. **Верификация email** — при привязке email в настройках отправлять код подтверждения, активировать только после ввода кода
2. **Сброс пароля** — кнопка "Забыли пароль?" на странице логина, отправка ссылки для сброса на привязанный email
3. **Уведомления на email** — об окончании подписки, успешной оплате (резервный канал если Telegram заблокирован)
