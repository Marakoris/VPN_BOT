...
Все пароли хранятся в **зашифрованном файле** (SOPS + age):
```bash
sops -d /root/claude-docs/projects/infrastructure/server-connections.enc.md
```
Содержит: SSH пароли серверов, Metabase VPN (admin@vpnbot.local), Metabase fotocccp, WinRM, Proxmox и др.

---

## 🌐 ДОМЕНЫ И СЕРВИСЫ

| Домен | Назначение | Где DNS | Примечание |
|-------|-----------|---------|------------|
| `fastnet-secure.com` | Лендинг (Cloudflare Pages) + Email (Resend SMTP) | Cloudflare | Основной публичный домен |
| `vpnnoborder.sytes.net` | Dashboard API, Subscription API | No-IP (DDNS) | Продакшн сервер |
| `analytic-vpn.sytes.net` | Metabase аналитика | No-IP (DDNS) | |
| `grafana.sytes.net` | Grafana мониторинг | No-IP (DDNS) | |
| `vault.friendadmin.ru` | Vaultwarden (менеджер паролей) | — | |

### Cloudflare (`fastnet-secure.com`)
- **Account**: Marakoris@gmail.com
- **Zone ID**: `614b154094048c48fc5e02e3bb10c37c`
- **Account ID**: `6d755f2fe5d90ac46b3d6e2f90de59a8`
- **API Token**: в Vaultwarden → "Cloudflare (Pages + DNS)"
- **Pages project**: `noborder-landing` → `noborder-landing.pages.dev`

### Email (Resend SMTP)
- **Сервис**: Resend.com (SMTP: `smtp.resend.com:587`)
- **API Key**: в Vaultwarden → "Resend SMTP (Email Service)"
- **From**: `noreply@fastnet-secure.com`
- **Лимит**: 100 писем/день (free tier)
- **Используется для**: верификация email, сброс пароля, уведомления об оплате/истечении

## ✅ Email-сервис (реализовано 2026-02-20)

- Верификация email при привязке и регистрации (6-значный код)
- Сброс пароля через email-ссылку
- Email-уведомления об оплате и истечении подписки (1/3 дня)
- Файлы: `email_service.py`, `email_templates.py` в `subscription_api/dashboard/`
