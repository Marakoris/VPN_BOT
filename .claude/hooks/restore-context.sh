...
Доступы к VPN серверам хранятся централизованно:
```bash
vpn-list                    # Список всех серверов
vpn-info <alias>            # Полная информация о сервере
vpn-ssh <alias>             # SSH подключение
vpn-ssh <alias> "command"   # Выполнить команду
vpn-password <alias> <pass> # Обновить пароль + синхронизация
vpn-edit <alias>            # Редактировать конфиг
```
...
vpn-list                    # Список всех серверов
vpn-ssh <alias>             # SSH подключение
vpn-info <alias>            # Полная информация
